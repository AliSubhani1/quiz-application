...
const QuizFirstPage = () => {
  ...
  useEffect(() => {
    if (quizData && currentPage !== "firstPage") {
      dispatch(setCurrentPage("firstPage"));
    }
  }, [quizData, currentPage, dispatch]);

  if (!quizData || currentPage !== "firstPage") return null;

  const progressPercentage = (2 / quizData.questions.length) * 100;
  ...
  const handleContinue = () => {
    // Check if answers are selected for both questions
    const unansweredQuestions = quizData.questions.slice(0, 2).filter(
      (question) => !selectedAnswers[question.id]
    );

    if (unansweredQuestions.length > 0) {
      setError("Please select an answer for all questions before continuing.");
      return;
    }

    dispatch(setCurrentPage("secondPage"));
  };

  return (
    <div className="bg-white-0 my-6 w-[80%] mx-auto min-h-[85vh]">
      <TitleSection heading={quizData.quizName} />
      <ProgressBar percentage={progressPercentage} currentQuestion={2} totalQuestions={4} />

      {quizData.questions.slice(0, 2).map((question) => (
        <div key={question.id} className="w-[90%] mx-auto mt-12">
